...
const getClassForHint = (state) => {
    ...
    return (hint) => {
        ...
        if (playHints.find(x => x === hint)) {
            ...
            switch (hint) {
                case PLAY:
                    if (!gamePhase.isWon && (gamePhase.playingPips || gamePhase.playingAce || gamePhase.playingJoker)) {
                        return ['active'];
                    }
                    break;
                case PIP:
...
export default function drawInstructions(state) {
    ...
    <ul>
      <li class="${getHintClass(WIN)}">WIN: All Royals have been knocked out</li>
      ...
      <li class="${getHintClass(LOSEOVERARMORED)}">LOSE: Too much armor on a royal</li>
    </ul>
    `;
