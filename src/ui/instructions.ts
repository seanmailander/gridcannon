import { getGamePhase } from "../app/game.selectors.js";
import { instructionIdentifiers } from "../app/game.consts.js";
...
export default function setInstructions(state) {
  ...
  setInstructionActive(
    instructionIdentifiers.ROYAL,
    gamePhase.playingRoyalty,
    gameOver
  );
  setInstructionActive(
    instructionIdentifiers.ARMOR,
    gamePhase.addingArmor,
    gameOver
  );
  ...
  // End
  setInstructionActive(
    instructionIdentifiers.END,
    gamePhase.isWon || gamePhase.isLost
  );
  setInstructionActive(instructionIdentifiers.WIN, gamePhase.isWon);
  setInstructionActive(
    instructionIdentifiers.LOSENOCARD,
    !gamePhase.isWon && gamePhase.noCardsRemaining
  );
  setInstructionActive(
    instructionIdentifiers.LOSENOROYAL,
    !gamePhase.isWon && gamePhase.noRoyal
  );
  setInstructionActive(
    instructionIdentifiers.LOSEOVERARMORED,
    !gamePhase.isWon && gamePhase.unwinnableArmor
  );
}
