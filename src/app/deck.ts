...
const repeat = (howMany, target) => [...Array(howMany).keys()].map(() => target);

export const shuffleDeck = ({ seed, kidding, harder }: { seed: string, kidding?: boolean, harder?: boolean }) => {
    ...
    const howManyJokersToAdd = harder ? withoutJokers : kidding ? withExtraJokers : defaultJokers;
