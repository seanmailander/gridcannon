...
export const openSpotsForNonRoyal = (state: IGameState) => {
    ...
    const cardCanClear = card === CARDS.ACE || card === JOKER || card === undefined;
    if (cardCanClear) {
        return playSpots;
    }

    const getTopMostCard = (spotIndex) => (grid[spotIndex][0] || { card: 0 }).card || 0;

    return playSpots.filter(
        (spotIndex) => getTopMostCard(spotIndex) <= card
    );
};
...
export const hasNoCardsRemaining = (state: IGameState) => state.deckInHand.length === 0;
...
export const canTimeTravel = (stateWithTimeTravel: StateWithHistory<IGameState>) => {
